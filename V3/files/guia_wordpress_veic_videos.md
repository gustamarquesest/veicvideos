...
## ATUALIZAÇÃO — VERSÃO 2 DO SITE

### Alterações aplicadas nesta versão

1. **Stroke (traço) nas letras Impact** — todas as letras grandes têm um traço preto da cor do fundo, que separa as letras visualmente sem mudar a cor.
2. **Processo atualizado para 5 fases** — Levantamento → Direção de Roteiro → Gravação com Direção → Edição → Entrega do Vídeo
3. **Número WhatsApp corrigido** — (31) 98333-5876 em todo o site
4. **Caixa de contato clicável** — o box "(31) 98333-5876 · Atendimento via WhatsApp" agora é um link clicável para o WhatsApp, além do botão existente
5. **Botão WhatsApp flutuante verde** — fixo no canto inferior direito em todas as telas
6. **Seção "Trabalhamos por Projetos"** — adicionada antes do CTA final com tags: Campanhas, Anúncios, Posicionamento, Eventos, Ações, Feiras, etc.
7. **Hamburger menu mobile** — navegação colapsável em telas menores que 900px
8. **Responsivo aprimorado** — breakpoints para 900px e 480px

---

## COMO ALTERAR O STROKE (TRAÇO) DAS LETRAS

O stroke está configurado no CSS com a propriedade `-webkit-text-stroke`. Para ajustar:

### Aumentar ou diminuir a espessura do traço

Localize no CSS do site (dentro da tag `<style>`):
...
Troque o valor `2px` pelo que desejar:
- `1px` — traço mais fino, letras parecem mais leves
- `2px` — padrão atual
- `3px` — traço mais grosso, letras mais separadas
- `0px` — remove o traço completamente

**No Elementor:** Em qualquer widget de Heading com fonte Impact, vá em **Estilo → Tipografia → CSS personalizado** e adicione:
```css
-webkit-text-stroke: 2px #131210;
paint-order: stroke fill;
```

### Alterar a gramatura (peso) das letras

Impact não tem variações de peso — ela é sempre Bold. Se quiser variação, troque para **Inter** com pesos diferentes:

No Elementor: Estilo → Tipografia → Peso da fonte:
- 400 = Regular (fino)
- 600 = SemiBold
- 700 = Bold
- 800 = ExtraBold
- 900 = Black (mais grosso)

---

## COMO GERENCIAR VÍDEOS NO SITE

### Como subir um vídeo no WordPress

1. Copie o ID do YouTube do vídeo (exemplo: `SIpWQ_e89oo`)
2. WordPress Admin → **Vídeos → Adicionar Novo**
3. Preencha título, campos ACF (URL, orientação, tipo, nicho)
4. Para a **capa/thumbnail**: vá na aba **"Imagem Destacada"** (canto inferior direito da tela de edição)
5. Clique em "Definir imagem destacada" → faça upload da capa

### Como capturar a capa (thumbnail) de um vídeo YouTube

**Opção 1 — Pegar thumbnail automática do YouTube:**
Acesse no navegador: `https://i.ytimg.com/vi/ID_DO_VIDEO/maxresdefault.jpg`
Substitua `ID_DO_VIDEO` pelo ID real. Clique com o botão direito → Salvar imagem.

**Opção 2 — Capturar um frame específico:**
1. Abra o vídeo no YouTube
2. Pause no frame desejado
3. Use **Kapwing.com** → "Screenshot" → cole a URL do vídeo → selecione o momento exato
4. Faça download da imagem

**Opção 3 — No próprio Elementor:**
O widget de vídeo do Elementor tem campo "Imagem de sobreposição" onde você faz upload de qualquer imagem como capa.

### Vídeos bloqueados por direitos autorais

Use o **Vimeo** (vimeo.com — plano gratuito):
1. Crie conta em vimeo.com
2. Faça upload do vídeo
3. Privacidade: "Only people with the private link"
4. Copie o link de embed
5. No Elementor, widget de vídeo → selecione "Vimeo" → cole o link

### Vídeos verticais longos (+3 minutos no YouTube)

No Elementor, ao inserir o widget de vídeo:
- Cole a URL normal do YouTube (não precisa ser Shorts)
- Clique em **"Proporção de aspecto"** → selecione **"9:16"** ou personalize
- O player do YouTube se adapta automaticamente ao formato vertical
