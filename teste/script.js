...
const moments = [
  {
    ...
    videos: ["Anúncios em vídeo", "Conteúdo para Instagram", "Reels e Shorts", "Vídeos educativos", "Teasers de impacto", "Conteúdo para engajar", "Conteúdo para gerar desejo"],
...
const recommendations = [
  ...
  },
  {
    ...
    pills: ["Apresentação do Negócio", "Apresentação da Equipe", "Missão, visão, valores", "Diferenciais", "Soluções", "Bastidores / cultura"],
  ...
  },
  {
    ...
    pills: ["Cases de Sucesso", "Conteúdos Institucionais", "Conteúdo educativo", "Conteúdo informativo", "Conteúdo para LinkedIn"],
  ...
  },
  {
    ...
    pills: ["VSL", "Vídeos comerciais", "Resultados alcançados", "Convite para agendamento", "Vídeos Comparativos", "Oferta personalizada", "Condição Especial / Desconto"],
    examples: ["cPl8z5m_nwY", "CgfEWgE_8Aw", "fAzmIuYKra0"],
  },
  {
    title: "Experiência e fidelização",
    desc: "Vídeos para manter clientes próximos e aumentar recorrência.",
    pills: ["Conteúdo exclusivo", "Cursos e mini cursos", "Comunidade / Grupo VIP", "Programas de fidelidade", "Clube de benefícios", "Atualizações e Novidades"],
    examples: ["PodfSqi6ru8", "1196948372", "XbhVd3a0sEI"],
  },
  {
    title: "Comunicação interna",
    desc: "Vídeos para alinhar equipes, processos e cultura interna.",
    pills: ["Vídeos de boas-vindas", "História da empresa", "Apresentação da empresa", "Cultura, propósito e valores", "Políticas internas", "Treinamento técnico", "Procedimentos e processos", "Boas práticas", "Cursos e certificações", "Comunicados", "Endomarketing", "Resultados", "Eventos internos", "Mudanças e engajamento", "Capacitação e treinamento"],
    examples: ["jeY8AvM-iUY", "1196950750", "BRhNBATj9Z0"],
  },
  {
    title: "Eventos, feiras e ações de campanhas",
    desc: "Vídeos para transformar ações presenciais em ativos de comunicação.",
    pills: ["Eventos", "Feiras", "Convenções", "Ativação de Marca", "Palestras", "Campanhas", "Lançamentos"],
...
function animateFeatured() {
  if (!featuredDragging && featuredHalfWidth > 0) {
    featuredOffset = normalizeFeaturedOffset(featuredOffset + (featuredBaseSpeed * featuredSpeedBoost * featuredDirection));
    featuredRail.style.transform = `translateX(${-featuredOffset}px)`;
  }
...
function setupFeaturedDrag() {
  ...
  featuredRail.addEventListener("pointermove", (event) => {
    ...
    featuredLastX = event.clientX;
    featuredOffset = normalizeFeaturedOffset(featuredOffset - (delta * 2.15));
    featuredRail.style.transform = `translateX(${-featuredOffset}px)`;
  });
  ...
  return `
    <div class="mobile-recommendation">
      <div class="recommendation">
        <button class="mobile-recommendation-close" type="button">Vídeos indicados ↑</button>
        ...
        <div class="recommendation-body">
          ...
          <div class="rec-examples">${item.examples.map((id) => {
            const video = findVideo(id);
            return `
              <button class="example-video ${video.o === "horizontal" ? "horizontal" : ""} js-open-video" type="button" data-id="${video.id}" data-platform="${video.p}" data-orientation="${video.o}" data-title="${video.tag} · ${video.title}">
                ${thumbUrl(video) ? `<img src="${thumbUrl(video)}" alt="Frame do vídeo ${video.title}" loading="lazy">` : `<div class="vimeo-fallback"><span>Vimeo</span></div>`}
...
function closeMobileRecommendation() {
  document.querySelectorAll(".mobile-recommendation").forEach((panel) => panel.remove());
