...
function setupRevealAnimations() {
  ...
  typeCards.forEach((card, index) => {
    card.style.transitionDelay = `${Math.min(index * 100, 520)}ms`;
    observer.observe(card);
  });

  processCards.forEach((card, index) => {
    card.style.transitionDelay = `${index * 195}ms`;
...
function openVideo({ id, platform, orientation, title, sound = false }) {
  ...
  const src = isVimeo
    ...
    : `https://www.youtube.com/embed/${id}?autoplay=1&mute=0&playsinline=1&rel=0`;
