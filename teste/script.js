...
function setupRevealAnimations() {
  const cards = document.querySelectorAll(".type-grid article, .process-rail article");
...
function setupPortfolioDrag() {
  ...
  portfolioCarousel.addEventListener("pointerdown", (event) => {
...
momentDots?.addEventListener("click", (event) => {
  ...
  if (!dot) return;
  const index = Number(dot.dataset.moment);
  const tab = document.querySelector(`.moment-tab[data-moment="${index}"]`);
  document.querySelectorAll(".moment-tab").forEach((item) => item.classList.remove("active"));
  tab?.classList.add("active");
  renderMoment(index);
  if (window.matchMedia("(max-width: 759px)").matches) {
