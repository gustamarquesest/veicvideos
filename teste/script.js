...
function thumbSrcset(video) {
  if (video.p === "vimeo") {
    return `https://vumbnail.com/${video.id}_large.jpg 640w, https://vumbnail.com/${video.id}.jpg 640w`;
  }
  return `https://img.youtube.com/vi/${video.id}/maxresdefault.jpg 1280w, https://img.youtube.com/vi/${video.id}/sddefault.jpg 640w, https://img.youtube.com/vi/${video.id}/hqdefault.jpg 480w`;
}

function thumbFallbackUrl(video) {
  if (video.p === "vimeo") return `https://vumbnail.com/${video.id}.jpg`;
  return `https://img.youtube.com/vi/${video.id}/hqdefault.jpg`;
}

function thumbImage(video, alt) {
  return `<img src="${thumbUrl(video)}" srcset="${thumbSrcset(video)}" sizes="(max-width: 759px) 86vw, 640px" alt="${alt}" loading="lazy" draggable="false" onerror="this.onerror=null;this.src='${thumbFallbackUrl(video)}';">`;
}
...
function diagnosticVisualHtml(title) {
  return `
    <div class="diagnostic-image-slot" aria-label="Espaço visual para ${title}">
      <div class="diagnostic-image-mark">
        <span></span>
      </div>
...
function renderRecommendation(index) {
  ...
  document.querySelector("#recExamples").innerHTML = diagnosticVisualHtml(item.title);
}
    ...
    <div class="mobile-recommendation">
      <div class="recommendation">
        ...
        <div class="recommendation-body">
          ...
          <div class="rec-examples">${diagnosticVisualHtml(item.title)}</div>
        </div>
...
function setupMomentMobileWheel() {
  ...
  function animatePanelSwitch(direction) {
    ...
    panelTrack.style.transition = "none";
    panelTrack.style.transform = `translateX(${momentPanelDirection * 18}px)`;
    panelTrack.style.opacity = ".72";
    requestAnimationFrame(() => {
      panelTrack.style.transition = "transform .34s cubic-bezier(.22,.88,.24,1), opacity .24s ease";
      panelTrack.style.transform = "translateX(0)";
      panelTrack.style.opacity = "1";
      window.setTimeout(() => panel.classList.remove("switching"), 360);
    });
  }
  ...
  panel.addEventListener("pointermove", (event) => {
    ...
    panelTrack.style.transform = `translateX(${Math.max(-56, Math.min(56, deltaX * .34))}px)`;
  ...
  function endPanelDrag(event) {
    if (!window.matchMedia("(max-width: 759px)").matches) return;
    function settle() {
      const active = Number(document.querySelector(".moment-tab.active")?.dataset.moment || 0);
      setMoment(active, true);
      panelTrack.style.transform = "translateX(0)";
...
function setupPortfolioDrag() {
  ...
  portfolioCarousel.addEventListener("pointermove", (event) => {
    ...
    portfolioDragLastX = event.clientX;
    if (Math.abs(portfolioDragLastX - portfolioDragStartX) > 4) {
      event.preventDefault();
    }
  });
  ...
  function endPortfolioDrag() {
    ...
    if (Math.abs(delta) > dragThreshold) {
      goToPortfolioSlide(portfolioSlideIndex + (delta < 0 ? 1 : -1));
...
function openVideo({ id, platform, orientation, title, sound = false }) {
  ...
  const src = isVimeo
    ? `https://player.vimeo.com/video/${id}?autoplay=1&badge=0&autopause=0&muted=0`
    : `https://www.youtube-nocookie.com/embed/${id}?autoplay=1&rel=0&modestbranding=1&playsinline=1&vq=hd1080&hd=1&quality=hd1080`;
