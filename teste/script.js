...
function closeMobileRecommendation({ animate = false, restore = false } = {}) {
  ...
  function finish() {
    ...
    if (restore) window.setTimeout(() => {
      window.scrollTo({ top: mobileDiagnosticReturnY, behavior: "smooth" });
    }, 40);
  }
...
document.querySelectorAll(".need").forEach((need) => {
  need.addEventListener("click", () => {
    if (window.matchMedia("(max-width: 759px)").matches) {
      ...
      if (isAlreadyOpen) {
        closeMobileRecommendation({ animate: true, restore: true });
        return;
      }

      mobileDiagnosticReturnY = window.scrollY;
