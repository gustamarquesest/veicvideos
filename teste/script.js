...
function renderFeatured() {
  featuredRail.innerHTML = featuredVideos.map((video) => card(video)).join("");
}

function filterKey(video) {
  const tag = video.tag.toLowerCase();
  if (tag.includes("case") || tag.includes("depoimento")) return "Cases";
  if (tag.includes("anúncio") || tag.includes("campanha") || tag.includes("teaser")) return "Anúncios";
  if (tag.includes("festival") || tag.includes("convite")) return "Eventos";
  if (tag.includes("institucional") || tag.includes("treinamento") || tag.includes("endomarketing")) return "Interno";
  if (tag.includes("produto") || tag.includes("serviço") || tag.includes("benefício")) return "Apresentação";
  if (tag.includes("educativo") || tag.includes("conteúdo")) return "Conteúdo";
  return "Todos";
}

function renderFilters() {
  const filters = ["Todos", "Cases", "Anúncios", "Apresentação", "Conteúdo", "Eventos", "Interno"];
  portfolioFilters.innerHTML = filters.map((filter) => (
    `<button class="filter ${filter === activeFilter ? "active" : ""}" type="button" data-filter="${filter}">${filter}</button>`
  )).join("");
}

function currentPortfolioVideos() {
  if (activeFilter === "Todos") return allVideos;
  return allVideos.filter((video) => filterKey(video) === activeFilter);
}

function renderPortfolio() {
  const videos = currentPortfolioVideos();
  portfolioGrid.innerHTML = videos.slice(0, visibleCount).map((video) => `
  ...
  `).join("");
  loadMoreButton.style.display = visibleCount >= videos.length ? "none" : "inline-flex";
}
...
function renderMoment(index) {
  ...
  renderExampleList(document.querySelector("#momentExamples"), item.examples);
}
...
function openVideo({ id, platform, orientation, title }) {
