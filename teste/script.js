...
function renderFeatured() {
  ...
  requestAnimationFrame(() => {
    ...
    featuredOffset = featuredOffset % featuredHalfWidth;
...
function animateFeatured() {
  if (!featuredDragging && featuredHalfWidth > 0) {
    featuredOffset = (featuredOffset + .55) % featuredHalfWidth;
    featuredRail.style.transform = `translateX(${-featuredOffset}px)`;
  }
...
function setupFeaturedDrag() {
  featuredMarquee.addEventListener("pointerdown", (event) => {
    featuredDragging = true;
    featuredMoved = false;
    featuredLastX = event.clientX;
    featuredMarquee.classList.add("dragging");
    featuredMarquee.setPointerCapture(event.pointerId);
  });

  featuredMarquee.addEventListener("pointermove", (event) => {
    ...
    featuredLastX = event.clientX;
    featuredOffset = (featuredOffset - delta) % featuredHalfWidth;
    if (featuredOffset < 0) featuredOffset += featuredHalfWidth;
    featuredRail.style.transform = `translateX(${-featuredOffset}px)`;
  });
  ...
  function endDrag(event) {
    ...
    if (event.pointerId !== undefined) {
      try { featuredMarquee.releasePointerCapture(event.pointerId); } catch (error) {}
    }
  }

  featuredMarquee.addEventListener("pointerup", endDrag);
  featuredMarquee.addEventListener("pointercancel", endDrag);
  featuredMarquee.addEventListener("pointerleave", endDrag);
}
