...
function openVideo({ id, platform, orientation, title, sound = false }) {
  ...
  let src;
  if (isVimeo) {
    src = `https://player.vimeo.com/video/${id}?autoplay=1&badge=0&autopause=0&muted=0#t=0s`;
  } else if (isMobile) {
    // No mobile, iniciar mutado para garantir autoplay e desmutar via postMessage logo depois
    src = `https://www.youtube.com/embed/${id}?autoplay=1&mute=1&enablejsapi=1&rel=0&modestbranding=1&playsinline=1&vq=hd720`;
  } else {
    src = `https://www.youtube.com/embed/${id}?autoplay=1&mute=0&enablejsapi=1&rel=0&modestbranding=1&playsinline=1&vq=hd1080&hd=1&quality=hd1080`;
  }
  ...
  iframe.src = src;
  iframe.title = title || "Vídeo Veic";
  iframe.allow = "autoplay; fullscreen; picture-in-picture; encrypted-media";
  iframe.allowFullscreen = true;
  modalVideoWrap.appendChild(iframe);

  // No mobile com YouTube: após o iframe carregar, desmutar via postMessage
  if (!isVimeo && isMobile) {
    function tryUnmute() {
      try {
    ...
    // Tenta várias vezes nos primeiros segundos para garantir que o player já carregou
    const delays = [600, 1200, 2000, 3200];
    delays.forEach((delay) => window.setTimeout(tryUnmute, delay));
  }
}
