...
function setupMomentMobileWheel() {
  ...
  function animatePanelSwitch(direction) {
    ...
    panelTrack.style.transform = `translateX(${momentPanelDirection * 10}px)`;
    panelTrack.style.opacity = ".86";
    requestAnimationFrame(() => {
      panelTrack.style.transition = "transform .24s cubic-bezier(.16,1,.3,1), opacity .18s ease";
  ...
  panel.addEventListener("pointermove", (event) => {
    ...
    panelTrack.style.transform = `translateX(${Math.max(-42, Math.min(42, deltaX * .22))}px)`;
...
function openVideo({ id, platform, orientation, title, sound = false }) {
  ...
  const src = isVimeo
    ...
    : `https://www.youtube-nocookie.com/embed/${id}?autoplay=1&mute=0&rel=0&modestbranding=1&playsinline=1&vq=hd1080&hd=1&quality=hd1080`;
