...
function setupMomentMobileWheel() {
  ...
  function animatePanelSwitch(direction) {
    ...
    panel.style.transform = `translateX(${momentPanelDirection * 10}px)`;
    panelTrack.style.opacity = ".86";
    requestAnimationFrame(() => {
      panel.style.transition = "transform .24s cubic-bezier(.16,1,.3,1)";
  ...
  panel.addEventListener("pointermove", (event) => {
    ...
    panel.style.transform = `translateX(${Math.max(-34, Math.min(34, deltaX * .18))}px)`;
